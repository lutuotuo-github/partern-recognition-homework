function [cost, grad, preds] = cnn_train2(cnn,minibatch_data,minibatch_labels,pred)
theta = cnn.theta;
images = minibatch_data;
labels = minibatch_labels;
numClasses = cnn.numClasses;
filterDim = cnn.layers{2, 1}.kernelsize;
numFilters = cnn.layers{2, 1}.outputmaps;
poolDim = cnn.layers{3, 1}.scale;
...
switch activationType
    case 'relu'
        errorsConvolution = errorsPooling .* (activations > 0);
        % relu derivative = x > 1
    case 'sigmoid'
